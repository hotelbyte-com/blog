...
description: "WP04 导读：数据库与存储韧性不是基础设施勾选项，而是应用层必须承担的契约。"
lang: zh
...
WP04 更适合按“控制设计”来读。它讨论的不是 HotelByte 有没有一个叫做“数据库与存储韧性层”的能力，而是这项能力在真实酒店分销环境里，如何划边界、保留事实、分类失败、证明结果，并在运营压力下仍然可解释。

**TL;DR：** 数据库与存储韧性不是基础设施勾选项，而是应用层必须承担的契约。
...
更好的问题不是“HotelByte 是否支持 数据库与存储韧性层”，而是：
...
HotelByte 把 数据库与存储韧性层 放在一套更大的受治理平台里，而不是把它当成孤立实现细节。架构需要留下痕迹：标准化契约、明确所有权、可测试行为，以及事后可以复查的生产证据。
...
- [中文白皮书原文](/zh/whitepapers/wp04-database-storage-resilience/original/)
- [English whitepaper](/en/whitepapers/wp04-database-storage-resilience/original/)
