...
description: "WP18 导读：运营 BI 需要面向时间窗口的证据层，而不是临时查询业务库。"
lang: zh
...
WP18 更适合按“控制设计”来读。它讨论的不是 HotelByte 有没有一个叫做“时序 BI 分析”的能力，而是这项能力在真实酒店分销环境里，如何划边界、保留事实、分类失败、证明结果，并在运营压力下仍然可解释。

**TL;DR：** 运营 BI 需要面向时间窗口的证据层，而不是临时查询业务库。
...
更好的问题不是“HotelByte 是否支持 时序 BI 分析”，而是：
...
HotelByte 把 时序 BI 分析 放在一套更大的受治理平台里，而不是把它当成孤立实现细节。架构需要留下痕迹：标准化契约、明确所有权、可测试行为，以及事后可以复查的生产证据。
