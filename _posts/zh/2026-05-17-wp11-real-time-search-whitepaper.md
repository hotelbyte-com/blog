...
| 控制点 | 用户价值 |
|---|---|
| 慢供应商不能阻塞整体搜索完成 | 防止交易状态被并发流程或重复请求破坏，让订单、取消和补偿路径可复查。 |
| 重复报价需要按规则合并或标注 | 防止交易状态被并发流程或重复请求破坏，让订单、取消和补偿路径可复查。 |
| 排序依据必须可解释 | 防止交易状态被并发流程或重复请求破坏，让订单、取消和补偿路径可复查。 |
| 供应商错误分类进入可观测指标 | 防止交易状态被并发流程或重复请求破坏，让订单、取消和补偿路径可复查。 |
| 搜索结果与后续验价状态保持关联 | 防止交易状态被并发流程或重复请求破坏，让订单、取消和补偿路径可复查。 |
...
请按 WP27 的同一条闭环阅读 实时搜索聚合：意图、证据、有边界的执行、验证，以及可沉淀的治理记忆。
