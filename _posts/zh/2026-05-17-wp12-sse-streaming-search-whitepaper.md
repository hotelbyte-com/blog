...
| 控制点 | 用户价值 |
|---|---|
| 事件顺序必须可被前端幂等合并 | 防止控制被口头约定替代，并让外部评审可以沿着证据复查。 |
| 错误供应商以结构化状态暴露而不是中断整个流 | 防止控制被口头约定替代，并让外部评审可以沿着证据复查。 |
| 超时供应商进入完成摘要 | 防止控制被口头约定替代，并让外部评审可以沿着证据复查。 |
| 客户端断连需要释放服务端资源 | 防止控制被口头约定替代，并让外部评审可以沿着证据复查。 |
| 缓存与流式结果边界明确 | 防止控制被口头约定替代，并让外部评审可以沿着证据复查。 |
