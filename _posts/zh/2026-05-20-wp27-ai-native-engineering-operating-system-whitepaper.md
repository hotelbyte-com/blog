...
传统工程指标仍重要，但 AI 原生系统需要额外指标。这里的指标不是为了包装 AI 产能，而是为了判断系统有没有真的降低沟通成本、提高证据质量，并把经验沉淀下来。

| 指标 | 观察什么 | HotelByte 当前粗略状态 |
|---|---|---|
| 上下文获取时间 | AI 智能体多快能收集足够扎实证据开始行动。 | 仍在建设中，但已经从“人反复补充背景”转向“先整理代码、日志、页面、发布状态和历史上下文”的证据包模式。 |
| 证据覆盖率 | AI 结论中有多少被代码、测试、日志或运行时数据支撑。 | 对生产相关问题，要求结论尽量绑定代码引用、命令输出、浏览器截图、接口响应或环境回读；覆盖率还不是统一数字，但证据缺口会被显式写出。 |
| 已验证变更延迟 | 从人类意图到已测试、可评审变更的时间。 | 内容、博客、白皮书这类低风险资产已经可以在同一闭环里完成“修改、构建、截图、发布、线上检查”；高风险后端变更仍需要更严格测试和环境验证。 |
| 评审闭环率 | 评审意见中有多少通过代码、测试和回复闭环。 | 正在从“提交代码后等待人跟进”转向“识别 review thread、补测试、push、回复、确认关闭”的闭环工作方式。 |
| 记忆转化率 | 重复纠偏中有多少沉淀为持久规则或文档。 | 已经开始把反复出现的工程偏好写入仓库规则、技能、白皮书和运行手册；例如 27 篇技术白皮书本身就是把系统能力显性化的一部分。 |
| 自动化逃逸率 | 智能体动作绕过预期确认或验证的次数。 | 目前重点不是追求全自动，而是把高风险动作保留在人类授权和证据门禁内；越权、无证据完成、跳过发布验证都应被视为失败。 |
| 人类判断负载 | 人类时间花在决策上还是机械执行上。 | 目标是在进行中：把上下文搬运、证据整理、机械修改和状态同步交给 AI，让人把注意力放在复杂度本质、业务风险和架构取舍上。 |

这些还不是成熟仪表盘上的最终 KPI，更像一组正在形成的运营信号。HotelByte 当前能分享的粗略结论是：AI 的价值不应该用“生成了多少代码”衡量，而应该看它是否减少了无效沟通，是否把证据补齐，是否让评审更快聚焦真正风险，是否把一次纠偏变成下一次可复用的规则。
...
采用路线之后，还需要明确边界。AI 原生工程操作系统不是越重越好，也不是越自治越好。本节说明什么时候值得引入这套控制面，以及哪些做法会把治理系统变成新的仪式负担。
