...
| 控制点 | 用户价值 |
|---|---|
| 规则变更不能绕过预览和审计 | 防止控制被口头约定替代，并让外部评审可以沿着证据复查。 |
| 成本侧和售价侧规则不得混淆 | 防止控制被口头约定替代，并让外部评审可以沿着证据复查。 |
| 价格输出保留命中路径 | 防止控制被口头约定替代，并让外部评审可以沿着证据复查。 |
| 异常规则不应影响无关租户或市场 | 防止控制被口头约定替代，并让外部评审可以沿着证据复查。 |
| 规则缺失时使用保守默认 | 防止控制被口头约定替代，并让外部评审可以沿着证据复查。 |
