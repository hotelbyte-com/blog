...
| 控制点 | 用户价值 |
|---|---|
| 不能从其他金额借用币种 | 防止控制被口头约定替代，并让外部评审可以沿着证据复查。 |
| 利润必须由后端提供金额和币种，否则前端/导出留空 | 防止控制被口头约定替代，并让外部评审可以沿着证据复查。 |
| 部分财务和不完整税费不能持久化为完整结果 | 防止控制被口头约定替代，并让外部评审可以沿着证据复查。 |
| 供应商备注中的必付费用需要结构化处理 | 防止控制被口头约定替代，并让外部评审可以沿着证据复查。 |
| 单房 rate 与整单 totalRate 不得混用 | 防止控制被口头约定替代，并让外部评审可以沿着证据复查。 |
