...
description: "WP01 guide: A hotel API gateway is not just ingress plumbing. It is the first governed control plane for authentication, authorization, caching, streaming, field shaping, and request evidence."
lang: en
...
---
# Whitepaper Guide: HTTP Gateway & In-Process API Routing
...
WP01 is different. It should be read as a control design for infrastructure: where the system draws boundaries, which facts must be preserved, how failure is classified, and what evidence proves the capability works.

**TL;DR:** A hotel API gateway is not just ingress plumbing. It is the first governed control plane for authentication, authorization, caching, streaming, field shaping, and request evidence.
...
The question behind this asset is not "does HotelByte have HTTP Gateway & In-Process API Routing?" The better question is:
...
HotelByte treats HTTP Gateway & In-Process API Routing as part of a broader governed platform, not as an isolated implementation detail. The architecture is expected to leave a trail: normalized contracts, explicit ownership, testable behavior, and production evidence that can be inspected after the fact.
